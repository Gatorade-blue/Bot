...
function createBot() {
   ...
   function getOnlinePlayers() {
     ...
     function startLonelyMessages() {
       if (lonelyInterval) return

       const sendMessage = () => {
         if (getOnlinePlayers().length === 0) {
           bot.chat("i am lonely somebody join")
         }
       }

       sendMessage()

       lonelyInterval = setInterval(
         sendMessage,
         (10 + Math.random() * 5) * 60 * 1000 // 10–15 minutes
       )
     }

     function stopLonelyMessages() {
       if (lonelyInterval) {
         clearInterval(lonelyInterval)
         lonelyInterval = null
       }
     }
